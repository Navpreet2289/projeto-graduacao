...
PATH_DST = 'C:\Users\Alexandre-Asus\Documents\projeto-graduacao\img\created-pos-10\\'
FILE_NAME_SRC = 'info-created.txt'
FILE_NAME_DST = 'info-all.txt'
FILE_NAME_DST2 = 'info-all-full.txt'

with open(PATH_DST + FILE_NAME_DST, 'w') as output_short:
    output_short.close()

with open(PATH_DST + FILE_NAME_DST2, 'w') as output_full:
    output_full.close()

# try:
folders = os.listdir(PATH_DST)
folders.remove(FILE_NAME_DST)
folders.remove(FILE_NAME_DST2)
...
for f in folders:
    ...
    with open(PATH_DST + f + '\\' + FILE_NAME_SRC, 'r') as info_input:
        lines = info_input.readlines()
        info_input.close()

    for l in lines:
        new_lines_short.append(f + '\\' + l)
        new_lines_full.append(PATH_DST + f + '\\' + l)

    with open(PATH_DST + FILE_NAME_DST, 'a') as output_short:
        output_short.writelines(new_lines_short)
        output_short.close()

    with open(PATH_DST + FILE_NAME_DST2, 'a') as output_full:
        output_full.writelines(new_lines_full)
        output_full.close()
